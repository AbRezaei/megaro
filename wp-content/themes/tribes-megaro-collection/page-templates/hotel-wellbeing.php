<?php
/**
 * Template Name: Hotel Wellbeing
 * Description: 
 */
?>
<?php get_header(); ?>
<style>
  :root {
    --color-primary: #98005D;
    --color-secondary: #EDDEE1;
    --color-mid: #E8C862;
    --color-tertiary: #4A182A;
  }
</style>

<nav class="fixed lg:top-28 top-20 left-0 z-10 w-full bg-neutral-linen border border-[#E5E5E5]">
  <div class="overflow-x-auto w-full">
    <ul class="flex justify-center min-w-full w-max lg:gap-10 gap-8 px-8">

      <li class="group">
        <a href="#"
           class="block text-body-md hover:opacity-70 py-4 border-b-2 border-transparent group-[.active]:border-primary">The
                                                                                                                         Megaro</a>
      </li>
      <li class="group">
        <a href="#"
           class="block text-body-md hover:opacity-70 py-4 border-b-2 border-transparent group-[.active]:border-primary">Rooms</a>
      </li>
      <li class="group">
        <a href="#"
           class="block text-body-md hover:opacity-70 py-4 border-b-2 border-transparent group-[.active]:border-primary">Eat
                                                                                                                         & Drink</a>
      </li>
      <li class="group active">
        <a href="#"
           class="block text-body-md hover:opacity-70 py-4 border-b-2 border-transparent group-[.active]:border-primary">Wellbeing</a>
      </li>
      <li class="group">
        <a href="#"
           class="block text-body-md hover:opacity-70 py-4 border-b-2 border-transparent group-[.active]:border-primary">Group
                                                                                                                         Booking</a>
      </li>
      <li class="group">
        <a href="#"
           class="block text-body-md hover:opacity-70 py-4 border-b-2 border-transparent group-[.active]:border-primary">Gallery</a>
      </li>
      <li class="group">
        <a href="#"
           class="block text-body-md hover:opacity-70 py-4 border-b-2 border-transparent group-[.active]:border-primary">Location</a>
      </li>

    </ul>
  </div>
</nav>

<main class="lg:pt-28 pt-20">

  <header class="w-full md:h-dvh flex flex-row md:flex-nowrap flex-wrap">

    <div class="md:w-1/2 w-full h-full md:order-1 order-2 relative bg-tertiary flex flex-col justify-center items-center md:py-4 py-16 after:content-[''] after:absolute md:after:top-1/2 after:-top-24 after:left-1/2 after:z-1 md:after:-translate-y-1/2 after:w-px md:after:h-4/5 after:h-[calc(100%+6rem)] after:bg-primary">
      <div class="container relative z-2">
        <div class="w-full lg:w-4/5 flex flex-col justify-center items-center bg-tertiary py-6 mx-auto">

          <p class="text-body-lg text-secondary text-center mb-4">UNWIND</p>
          <h1 class="text-heading-1 text-white text-center mb-4">IN-ROOM <BR> WELLBEING</h1>
          <p class="text-body-lg text-[#E5E5E5] text-center mb-10">
            Our approach to wellness is focused on privacy and personalisation, whether you’re looking to
            unwind, get fit, or just treat yourself.
          </p>
          <a href="#" class="btn btn-primary-fill btn-lg">Make an enquiry</a>

        </div>
      </div>
    </div>
    <div class="md:w-1/2 w-full md:h-full h-100 md:order-2 order-1">
      <img src="<?= get_template_directory_uri() ?>/assets/img/Image Container.png" alt="Hero image" class="w-full h-full object-cover object-center">
    </div>

  </header>
  <section class="bg-secondary lg:py-24 py-16">
    <div class="container">
      <div class="lg:w-2/3 mx-auto">
        <h4 class="text-heading-4 text-center text-[#404040]">
          We offer a variety of relaxing treatments, from manicures to advanced facials, personal training,
          massages, and beauty and hair services. What’s more, it’s all in the comfort of your room, and can
          be booked easily through our concierge team all the way up to two hours before your desired time.
        </h4>
      </div>
    </div>
  </section>
  <section class="bg-neutral-linen lg:py-24 py-16">
    <div class="container">
      <div class="flex flex-row lg:flex-nowrap flex-wrap justify-end xl:gap-x-24 gap-x-12 gap-y-16">

        <div class="lg:w-2/5 w-full lg:order-1 order-2">

          <p class="text-body-lg text-primary mb-6">OVERLINE</p>
          <h2 class="text-heading-2 lg:mb-10 mb-8">MASSAGES</h2>
          <p class="text-body-lg text-[#404040] lg:mb-10 mb-8">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vestibulum felis ligula, feugiat sed mi
            eget, malesuada placerat sem.
          </p>
          <div class="overflow-x-auto lg:mb-10 mb-8">
            <table class="min-w-full text-left text-body-md">

              <thead class="whitespace-nowrap border-b border-gray-300">
              <tr>

                <th scope="col" class="min-w-48 py-3 font-normal">Massages</th>
                <th scope="col" class="min-w-16 py-3 font-normal">60 mins</th>
                <th scope="col" class="min-w-16 py-3 font-normal">90 mins</th>
                <th scope="col" class="min-w-16 py-3 font-normal">120 mins</th>

              </tr>
              </thead>
              <tbody>

              <tr class="border-b border-gray-300">
                <td class="py-3">Swedish massage</td>
                <td class="py-3">£130</td>
                <td class="py-3">£195</td>
                <td class="py-3">£260</td>
              </tr>
              <tr class="border-b border-gray-300">
                <td class="py-3">Deep tissue massage</td>
                <td class="py-3">£130</td>
                <td class="py-3">£195</td>
                <td class="py-3">£260</td>
              </tr>
              <tr class="border-b border-gray-300">
                <td class="py-3">Sports massage</td>
                <td class="py-3">£140</td>
                <td class="py-3">£210</td>
                <td class="py-3">£280</td>
              </tr>
              <tr class="border-b border-gray-300">
                <td class="py-3">Temple Spa detox & aromatherapy massage</td>
                <td class="py-3">£140</td>
                <td class="py-3">£210</td>
                <td class="py-3">£280</td>
              </tr>
              <tr class="border-b border-gray-300">
                <td class="py-3">Temple Spa relaxing massage</td>
                <td class="py-3">£140</td>
                <td class="py-3">£210</td>
                <td class="py-3">£280</td>
              </tr>
              <tr class="border-b border-gray-300">
                <td class="py-3">Pregnancy massage</td>
                <td class="py-3">£140</td>
                <td class="py-3">£210</td>
                <td class="py-3">£280</td>
              </tr>
              <tr class="border-b border-gray-300">
                <td class="py-3">Lymphatic drainage massage</td>
                <td class="py-3">£140</td>
                <td class="py-3">£210</td>
                <td class="py-3">£280</td>
              </tr>
              <tr class="border-b border-gray-300">
                <td class="py-3">Reflexology massage</td>
                <td class="py-3">£140</td>
                <td class="py-3">£210</td>
                <td class="py-3">£280</td>
              </tr>
              <tr class="border-b border-gray-300">
                <td class="py-3">Thai massage</td>
                <td class="py-3">£140</td>
                <td class="py-3">£210</td>
                <td class="py-3">£280</td>
              </tr>

              </tbody>

            </table>
          </div>
          <a href="#" class="btn btn-primary-fill btn-lg">Make an enquiry</a>

        </div>
        <div class="lg:w-3/5 md:w-4/5 w-full lg:order-2 order-1">

          <div class="relative z-1 flex flex-row justify-end">
            <img src="<?= get_template_directory_uri() ?>/assets/img/sample-16.jpg" alt="Sample"
                 class="w-4/5 aspect-square object-cover object-center"/>
          </div>
          <div class="relative z-1 flex flex-row justify-start md:-mt-32 -mt-20">
            <img src="<?= get_template_directory_uri() ?>/assets/img/sample-17.jpg" alt="Sample"
                 class="lg:w-2/5 w-1/2 aspect-square md:translate-x-0 translate-x-1/4 border-8 border-neutral-linen object-cover object-center"/>
          </div>

        </div>

      </div>
    </div>
  </section>
  <section class="bg-neutral-linen lg:py-24 py-16">
    <div class="container">
      <div class="flex flex-row lg:flex-nowrap flex-wrap justify-start xl:gap-x-24 gap-x-12 gap-y-16">

        <div class="lg:w-2/5 w-full lg:order-2 order-2">

          <p class="text-body-lg text-primary mb-6">OVERLINE</p>
          <h2 class="text-heading-2 lg:mb-10 mb-8">FACIALS</h2>
          <p class="text-body-lg text-[#404040] lg:mb-10 mb-8">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vestibulum felis ligula, feugiat sed mi
            eget, malesuada placerat sem.
          </p>
          <div class="overflow-x-auto lg:mb-10 mb-8">
            <table class="min-w-full text-left text-body-md">

              <thead class="whitespace-nowrap border-b border-gray-300">
              <tr>

                <th scope="col" class="min-w-48 py-3 font-normal">Facials</th>
                <th scope="col" class="min-w-16 py-3 font-normal">60 mins</th>
                <th scope="col" class="min-w-16 py-3 font-normal">90 mins</th>

              </tr>
              </thead>
              <tbody>

              <tr class="border-b border-gray-300">

                <td class="py-3">My Kinda Skin facial</td>
                <td class="py-3">£130</td>
                <td class="py-3">£195</td>

              </tr>
              <tr class="border-b border-gray-300">

                <td class="py-3">Go Guy facial</td>
                <td class="py-3">£130</td>
                <td class="py-3">£195</td>

              </tr>

              </tbody>

            </table>
          </div>
          <a href="#" class="btn btn-primary-fill btn-lg">Make an enquiry</a>

        </div>
        <div class="lg:w-3/5 md:w-4/5 w-full lg:order-1 order-1">

          <div class="relative z-1 flex flex-row justify-start">
            <img src="<?= get_template_directory_uri() ?>/assets/img/sample-18.jpg" alt="Sample"
                 class="w-4/5 aspect-square object-cover object-center"/>
          </div>
          <div class="relative z-1 flex flex-row justify-end md:-mt-32 -mt-20">
            <img src="<?= get_template_directory_uri() ?>/assets/img/sample-19.jpg" alt="Sample"
                 class="lg:w-2/5 w-1/2 aspect-square md:translate-x-0 -translate-x-1/4 border-8 border-neutral-linen object-cover object-center"/>
          </div>

        </div>

      </div>
    </div>
  </section>
  <section class="bg-neutral-linen lg:py-24 py-16">
    <div class="container">
      <div class="flex flex-row lg:flex-nowrap flex-wrap justify-end xl:gap-x-24 gap-x-12 gap-y-16">

        <div class="lg:w-2/5 w-full lg:order-1 order-2">

          <p class="text-body-lg text-primary mb-6">OVERLINE</p>
          <h2 class="text-heading-2 lg:mb-10 mb-8">MANI PEDI</h2>
          <p class="text-body-lg text-[#404040] lg:mb-10 mb-8">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vestibulum felis ligula, feugiat sed mi
            eget, malesuada placerat sem.
          </p>
          <div class="overflow-x-auto lg:mb-10 mb-8">
            <table class="min-w-full text-left text-body-md">

              <thead class="whitespace-nowrap border-b border-gray-300">
              <tr>

                <th scope="col" class="min-w-48 py-3 font-normal">Mani Pedi</th>
                <th scope="col" class="min-w-16 py-3 font-normal"></th>

              </tr>
              </thead>
              <tbody>

              <tr class="border-b border-gray-300">

                <td class="py-3">Luxury manicure or pedicure</td>
                <td class="py-3">£100 per hour</td>

              </tr>
              <tr class="border-b border-gray-300">

                <td class="py-3">Deep tissue massage</td>
                <td class="py-3">£100 per hour</td>

              </tr>
              <tr class="border-b border-gray-300">

                <td class="py-3">Sports massage</td>
                <td class="py-3">£40</td>

              </tr>
              <tr class="border-b border-gray-300">

                <td class="py-3">Temple Spa detox & aromatherapy massage</td>
                <td class="py-3">£30</td>

              </tr>

              </tbody>

            </table>
          </div>
          <a href="#" class="btn btn-primary-fill btn-lg">Make an enquiry</a>

        </div>
        <div class="lg:w-3/5 md:w-4/5 w-full lg:order-2 order-1">

          <div class="relative z-1 flex flex-row justify-end">
            <img src="<?= get_template_directory_uri() ?>/assets/img/sample-20.jpg" alt="Sample"
                 class="w-4/5 aspect-square object-cover object-center"/>
          </div>
          <div class="relative z-1 flex flex-row justify-start md:-mt-32 -mt-20">
            <img src="<?= get_template_directory_uri() ?>/assets/img/sample-21.jpg" alt="Sample"
                 class="lg:w-2/5 w-1/2 aspect-square md:translate-x-0 translate-x-1/4 border-8 border-neutral-linen object-cover object-center"/>
          </div>

        </div>

      </div>
    </div>
  </section>
  <section class="bg-neutral-linen lg:py-24 py-16">
    <div class="container">
      <div class="flex flex-row lg:flex-nowrap flex-wrap justify-start xl:gap-x-24 gap-x-12 gap-y-16">

        <div class="lg:w-2/5 w-full lg:order-2 order-2">

          <p class="text-body-lg text-primary mb-6">OVERLINE</p>
          <h2 class="text-heading-2 lg:mb-10 mb-8">HAIR & MAKEUP</h2>
          <p class="text-body-lg text-[#404040] lg:mb-10 mb-8">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vestibulum felis ligula, feugiat sed mi
            eget, malesuada placerat sem.
          </p>
          <div class="overflow-x-auto lg:mb-10 mb-8">
            <table class="min-w-full text-left text-body-md">

              <thead class="whitespace-nowrap border-b border-gray-300">
              <tr>

                <th scope="col" class="min-w-48 py-3 font-normal">Hair and Makeup</th>
                <th scope="col" class="min-w-16 py-3 font-normal"></th>

              </tr>
              </thead>
              <tbody>

              <tr class="border-b border-gray-300">

                <td class="py-3">Blow dry</td>
                <td class="py-3">£120</td>

              </tr>
              <tr class="border-b border-gray-300">

                <td class="py-3">Up styling</td>
                <td class="py-3">£120</td>

              </tr>
              <tr class="border-b border-gray-300">

                <td class="py-3">Makeup</td>
                <td class="py-3">£120</td>

              </tr>

              </tbody>

            </table>
          </div>
          <a href="#" class="btn btn-primary-fill btn-lg">Make an enquiry</a>

        </div>
        <div class="lg:w-3/5 md:w-4/5 w-full lg:order-1 order-1">

          <div class="relative z-1 flex flex-row justify-start">
            <img src="<?= get_template_directory_uri() ?>/assets/img/sample-22.jpg" alt="Sample"
                 class="w-4/5 aspect-square object-cover object-center"/>
          </div>
          <div class="relative z-1 flex flex-row justify-end md:-mt-32 -mt-20">
            <img src="<?= get_template_directory_uri() ?>/assets/img/sample-23.jpg" alt="Sample"
                 class="lg:w-2/5 w-1/2 aspect-square md:translate-x-0 -translate-x-1/4 border-8 border-neutral-linen object-cover object-center"/>
          </div>

        </div>

      </div>
    </div>
  </section>
  <section class="bg-neutral-linen lg:py-24 py-16">
    <div class="container">
      <div class="flex flex-row lg:flex-nowrap flex-wrap justify-end xl:gap-x-24 gap-x-12 gap-y-16">

        <div class="lg:w-2/5 w-full lg:order-1 order-2">

          <p class="text-body-lg text-primary mb-6">OVERLINE</p>
          <h2 class="text-heading-2 lg:mb-10 mb-8">FITNESS</h2>
          <p class="text-body-lg text-[#404040] lg:mb-10 mb-8">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vestibulum felis ligula, feugiat sed mi
            eget, malesuada placerat sem.
          </p>
          <div class="overflow-x-auto lg:mb-10 mb-8">
            <table class="min-w-full text-left text-body-md">

              <thead class="whitespace-nowrap border-b border-gray-300">
              <tr>

                <th scope="col" class="min-w-48 py-3 font-normal">Fitness</th>
                <th scope="col" class="min-w-16 py-3 font-normal"></th>

              </tr>
              </thead>
              <tbody>

              <tr class="border-b border-gray-300">

                <td class="py-3">Personal training</td>
                <td class="py-3">£130 per hour</td>

              </tr>
              <tr class="border-b border-gray-300">

                <td class="py-3">Pilates</td>
                <td class="py-3">£130 per hour</td>

              </tr>
              <tr class="border-b border-gray-300">

                <td class="py-3">Yoga</td>
                <td class="py-3">£130 per hour</td>

              </tr>

              </tbody>

            </table>
          </div>
          <a href="#" class="btn btn-primary-fill btn-lg">Make an enquiry</a>

        </div>
        <div class="lg:w-3/5 md:w-4/5 w-full lg:order-2 order-1">

          <div class="relative z-1 flex flex-row justify-end">
            <img src="<?= get_template_directory_uri() ?>/assets/img/sample-24.jpg" alt="Sample"
                 class="w-4/5 aspect-square object-cover object-center"/>
          </div>
          <div class="relative z-1 flex flex-row justify-start md:-mt-32 -mt-20">
            <img src="<?= get_template_directory_uri() ?>/assets/img/sample-25.jpg" alt="Sample"
                 class="lg:w-2/5 w-1/2 aspect-square md:translate-x-0 translate-x-1/4 border-8 border-neutral-linen object-cover object-center"/>
          </div>

        </div>

      </div>
    </div>
  </section>
  <section class="bg-[#F1EDE4] lg:py-24 py-16">
    <div class="container">
      <div class="lg:w-1/2 mx-auto">

        <div class="mb-16">

          <h3 class="text-heading-3 text-center mb-6">BOOKING ENQUIRY</h3>
          <p class="text-body-lg text-[#404040] text-center">Tortor interdum condimentum nunc molestie.</p>

        </div>
        <form action="#" class="grid grid-cols-1 sm:grid-cols-2 lg:gap-10 gap-8">

          <div class="sm:col-span-2">

            <p class="text-body-sm mb-2">Service</p>
            <div x-data="select()" class="relative">

              <select x-ref="selectInput" name="service_id" class="hidden">

                <option value="" disabled selected>Massage</option>
                <option value="1">This is a test - 1</option>
                <option value="2">This is a test - 2</option>
                <option value="3">This is a test - 3</option>
                <option value="4">This is a test - 4</option>
                <option value="5">This is a test - 5</option>
                <option value="6">This is a test - 6</option>
                <option value="7">This is a test - 7</option>

              </select>
              <div x-bind="trigger"
                   class="h-15 flex flex-col justify-center bg-white border px-4 relative cursor-pointer select-none duration-300"
                   :class="open ? 'border-primary' : 'border-secondary'">
                <div class="flex justify-between items-center">

                  <div class="flex items-center gap-2">

                    <p x-text="display" class="text-body-lg"
                       :class="display === options[0]?.label ? 'text-[#737373]' : ''"></p>

                  </div>
                  <img src="<?= get_template_directory_uri() ?>/assets/img/svg/chevron-down-black.svg" alt="Arrow icon"
                       x-bind:class="{'rotate-180': open}"
                       class="w-6 h-6 duration-300">

                </div>
              </div>
              <div x-bind="dialogue" x-cloak
                   class="absolute top-full left-0 w-full z-1 bg-white mt-1 max-h-60 overflow-auto">
                <template x-for="item in options" :key="item.value">
                  <div>

                    <template x-if="item.disabled">
                      <div x-text="item.label"
                           class="text-body-lg text-gray-500 py-2 px-4 cursor-not-allowed"></div>
                    </template>
                    <template x-if="!item.disabled">
                      <div x-bind="option(item)" x-text="item.label"
                           class="text-body-lg py-2 px-4 hover:bg-primary/10 cursor-pointer duration-300"></div>
                    </template>

                  </div>
                </template>
              </div>

            </div>

          </div>
          <div class="sm:col-span-2">

            <p class="text-body-sm mb-2">Name</p>
            <input type="text" placeholder="Enter your Name" class="w-full">

          </div>
          <div>

            <p class="text-body-sm mb-2">Email</p>
            <input type="text" placeholder="Enter your Email" class="w-full">

          </div>
          <div>

            <p class="text-body-sm mb-2">Phone</p>
            <input type="text" placeholder="Enter your Phone" class="w-full">

          </div>
          <div class="sm:col-span-2">

            <p class="text-body-sm mb-2">Message</p>
            <textarea placeholder="Enter your Message" class="w-full"></textarea>

          </div>
          <div class="sm:col-span-2">
            <button class="btn btn-primary-fill btn-lg">Make enquiry</button>
          </div>
          <div class="sm:col-span-2">
            <p class="text-body-md">By subscribing, you agree to our Terms & Conditions</p>
          </div>

        </form>

      </div>
    </div>
  </section>
  <section class="bg-secondary py-16">
    <div class="container">
      <div class="lg:w-2/3 xl:w-1/2 mx-auto">

        <div class="mb-6">

          <p class="text-body-lg text-primary text-center mb-4">SUBSCRIBE</p>
          <h3 class="text-heading-3 text-center mb-4">NEWSLETTER</h3>
          <p class="text-body-lg text-center text-[#404040]">Be the first to hear about great offers, new
                                                             openings and events.</p>

        </div>
        <div class="flex flex-row justify-center items-center mb-6">

          <input type="text" class="lg:w-80 w-56 text-body-lg" placeholder="Email address">
          <button class="btn btn-primary-fill btn-lg">Subscribe</button>

        </div>
        <p class="text-body-md text-center text-[#737373]">By subscribing, you agree to our Terms &
                                                           Conditions</p>

      </div>
    </div>
  </section>

</main>


<?php get_footer(); ?>
