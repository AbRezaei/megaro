<?php
/**
 * Template Name: Hotel Booking
 * Description: 
 */
?>
<?php get_header(); ?>
<style>
  :root {
    --color-primary: #98005D;
    --color-secondary: #EDDEE1;
    --color-mid: #E8C862;
    --color-tertiary: #4A182A;
  }
</style>


<nav class="fixed lg:top-28 top-20 left-0 z-10 w-full bg-neutral-linen border border-[#E5E5E5]">
  <div class="overflow-x-auto w-full">
    <ul class="flex justify-center min-w-full w-max lg:gap-10 gap-8 px-8">

      <li class="group">
        <a href="#"
           class="block text-body-md hover:opacity-70 py-4 border-b-2 border-transparent group-[.active]:border-primary">The
                                                                                                                         Megaro</a>
      </li>
      <li class="group">
        <a href="#"
           class="block text-body-md hover:opacity-70 py-4 border-b-2 border-transparent group-[.active]:border-primary">Rooms</a>
      </li>
      <li class="group">
        <a href="#"
           class="block text-body-md hover:opacity-70 py-4 border-b-2 border-transparent group-[.active]:border-primary">Eat
                                                                                                                         & Drink</a>
      </li>
      <li class="group">
        <a href="#"
           class="block text-body-md hover:opacity-70 py-4 border-b-2 border-transparent group-[.active]:border-primary">Wellbeing</a>
      </li>
      <li class="group active">
        <a href="#"
           class="block text-body-md hover:opacity-70 py-4 border-b-2 border-transparent group-[.active]:border-primary">Group
                                                                                                                         Booking</a>
      </li>
      <li class="group">
        <a href="#"
           class="block text-body-md hover:opacity-70 py-4 border-b-2 border-transparent group-[.active]:border-primary">Gallery</a>
      </li>
      <li class="group">
        <a href="#"
           class="block text-body-md hover:opacity-70 py-4 border-b-2 border-transparent group-[.active]:border-primary">Location</a>
      </li>

    </ul>
  </div>
</nav>

<main class="lg:pt-28 pt-20">

  <header class="bg-secondary lg:py-24 py-16">
    <div class="container">
      <div class="lg:w-1/2 mx-auto">

        <p class="text-body-lg text-primary text-center mb-4">STAY TOGETHER</p>
        <h1 class="text-heading-1 text-center mb-4">GROUP BOOKING</h1>
        <p class="text-body-lg text-center text-[#404040] mb-10">
          Travelling as a group of six or more? Whether you’re celebrating, working or exploring the city
          together, our hotel offers everything you need for a relaxed and memorable group stay.
        </p>
        <div class="flex flex-row justify-center">
          <a href="#" class="btn btn-primary-fill btn-lg">Make an enquiry</a>
        </div>

      </div>
    </div>
  </header>
  <section class="bg-neutral-linen lg:py-24 py-16">
    <div class="container">
      <div class="flex flex-row lg:flex-nowrap flex-wrap justify-end xl:gap-x-24 gap-x-12 gap-y-16">

        <div class="lg:w-2/5 w-full lg:order-1 order-2">

          <p class="text-body-lg text-primary mb-6">STAY TOGETHER</p>
          <h2 class="text-heading-2 lg:mb-10 mb-8">THE PERFECT STAY FOR GROUPS</h2>
          <p class="text-body-lg text-[#404040]">
            Staying with a larger group should feel effortless. That’s why our hotel offers thoughtful,
            flexible room options ideal for <b>six guests or more</b>, along with great on-site dining and a
            location that puts the best of the city right on your doorstep.
            <br>
            <br>
            Whether you’re here for a celebration, a business trip or a weekend escape with friends, we make
            group travel simple, comfortable and memorable.
          </p>

        </div>
        <div class="lg:w-3/5 md:w-4/5 w-full lg:order-2 order-1">

          <div class="relative z-1 flex flex-row justify-end">
            <img src="<?= get_template_directory_uri() ?>/assets/img/sample-26.jpg" alt="Sample"
                 class="w-4/5 aspect-square object-cover object-center"/>
          </div>
          <div class="relative z-1 flex flex-row justify-start md:-mt-32 -mt-20">
            <img src="<?= get_template_directory_uri() ?>/assets/img/hero-sample-1.jpg" alt="Sample"
                 class="lg:w-2/5 w-1/2 aspect-square md:translate-x-0 translate-x-1/4 border-8 border-neutral-linen object-cover object-center"/>
          </div>

        </div>

      </div>
    </div>
  </section>
  <section class="bg-tertiary lg:py-24 py-16">

    <div class="container">
      <div class="w-full lg:w-1/2 lg:mb-16 mb-12 mx-auto">

        <h3 class="text-heading-3 text-white text-center lg:mb-10 mb-8">SPACES TO GATHER</h3>
        <p class="text-body-lg text-[#E5E5E5] text-center">
          Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam eu nulla vitae velit condimentum
          interdum sed eu arcu. Maecenas lectus tellus, gravida at vestibulum vel, condimentum id nulla.
        </p>

      </div>
    </div>
    <div class="text-white">
      <div class="swiper relative" data-slider="autoHeight">

        <div class="swiper-wrapper">

          <div class="swiper-slide">
            <div class="w-full lg:h-118 h-75 flex flex-col justify-end">
              <img src="<?= get_template_directory_uri() ?>/assets/img/hero-sample-2.jpg" alt="Sample"
                   class="w-full h-auto max-h-full object-cover object-center">
            </div>
          </div>
          <div class="swiper-slide">
            <div class="w-full lg:h-118 h-75 flex flex-col justify-end">
              <img src="<?= get_template_directory_uri() ?>/assets/img/sample-28.jpg" alt="Sample"
                   class="w-full h-auto max-h-full object-cover object-center">
            </div>
          </div>
          <div class="swiper-slide">
            <div class="w-full lg:h-118 h-75 flex flex-col justify-end">
              <img src="<?= get_template_directory_uri() ?>/assets/img/sample-27.jpg" alt="Sample"
                   class="w-full h-auto max-h-full object-cover object-center">
            </div>
          </div>
          <div class="swiper-slide">
            <div class="w-full lg:h-118 h-75 flex flex-col justify-end">
              <img src="<?= get_template_directory_uri() ?>/assets/img/sample-14.jpg" alt="Sample"
                   class="w-full h-auto max-h-full object-cover object-center">
            </div>
          </div>
          <div class="swiper-slide">
            <div class="w-full lg:h-118 h-75 flex flex-col justify-end">
              <img src="<?= get_template_directory_uri() ?>/assets/img/sample-15.jpg" alt="Sample"
                   class="w-full h-auto max-h-full object-cover object-center">
            </div>
          </div>

        </div>
        <button data-swiper-button-previous
                class="absolute top-1/2 -translate-y-1/2 left-6 z-1 w-10 h-10 flex justify-center items-center bg-white/70 rounded-full hover:bg-white/90">
          <img src="<?= get_template_directory_uri() ?>/assets/img/svg/chevron-left-black.svg" alt="Arrow icon" class="w-4 h-4">
        </button>
        <button data-swiper-button-next
                class="absolute top-1/2 -translate-y-1/2 right-6 z-1 w-10 h-10 flex justify-center items-center bg-white/70 rounded-full hover:bg-white/90">
          <img src="<?= get_template_directory_uri() ?>/assets/img/svg/chevron-right-black.svg" alt="Arrow icon"
               class="w-4 h-4">
        </button>
        <div data-swiper-pagination class="w-fit! flex items-center gap-2 mx-auto mt-6 pb-2"></div>

      </div>
    </div>

  </section>
  <section class="bg-neutral-linen lg:py-24 py-16">
    <div class="container">

      <div class="lg:w-1/2 lg:mb-16 mb-12 mx-auto">
        <h3 class="text-heading-3 text-center">WHY BOOK YOUR <br> GROUP STAY WITH US</h3>
      </div>
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">

        <div class="flex flex-col">

          <div class="mb-6">
            <img src="<?= get_template_directory_uri() ?>/assets/img/sample-40.jpg" alt="Sample"
                 class="w-full sm:h-65 h-55 object-cover object-center">
          </div>
          <div class="grow flex flex-col">

            <div class="grow mb-6">

              <h5 class="text-heading-5 mb-6">Central location</h5>
              <p class="text-body-lg text-[#404040]">
                Stay moments from King’s Cross, with fast connections across London and the UK - ideal
                for groups arriving from different places or planning to explore the city together.
              </p>

            </div>

          </div>

        </div>
        <div class="flex flex-col">

          <div class="mb-6">
            <img src="<?= get_template_directory_uri() ?>/assets/img/sample-41.jpg" alt="Sample"
                 class="w-full sm:h-65 h-55 object-cover object-center">
          </div>
          <div class="grow flex flex-col">

            <div class="grow mb-6">

              <h5 class="text-heading-5 mb-6">Spaces to gather</h5>
              <p class="text-body-lg text-[#404040]">
                Whether you’re catching up, celebrating or planning the day ahead, our cosy lounge areas
                and social corners offer the perfect spots to gather as a group.
              </p>

            </div>

          </div>

        </div>
        <div class="flex flex-col">

          <div class="mb-6">
            <img src="<?= get_template_directory_uri() ?>/assets/img/sample-27.jpg" alt="Sample"
                 class="w-full sm:h-65 h-55 object-cover object-center">
          </div>
          <div class="grow flex flex-col">

            <div class="grow mb-6">

              <h5 class="text-heading-5 mb-6">On-site restaurant & bar</h5>
              <p class="text-body-lg text-[#404040]">
                Enjoy breakfast, snacks and evening drinks right here at the hotel, with relaxed spaces
                that make it easy for groups to eat, chat and unwind together.
              </p>

            </div>

          </div>

        </div>

      </div>

    </div>
  </section>
  <section class="bg-[#F1EDE4] lg:py-24 py-16">
    <div class="container">
      <div class="lg:w-1/2 mx-auto">

        <div class="mb-16">

          <p class="text-body-lg text-primary text-center mb-6">OVERLINE</p>
          <h3 class="text-heading-3 text-center mb-6">GROUP BOOKING ENQUIRY</h3>
          <p class="text-body-lg text-[#404040] text-center">Tortor interdum condimentum nunc molestie.</p>

        </div>
        <form action="#" class="grid grid-cols-1 sm:grid-cols-2 lg:gap-10 gap-8">

          <div class="sm:col-span-2">

            <p class="text-body-sm mb-2">Name</p>
            <input type="text" placeholder="Enter your Name" class="w-full">

          </div>
          <div>

            <p class="text-body-sm mb-2">Email</p>
            <input type="text" placeholder="Enter your Email" class="w-full">

          </div>
          <div>

            <p class="text-body-sm mb-2">Phone</p>
            <input type="text" placeholder="Enter your Phone" class="w-full">

          </div>
          <div>

            <p class="text-body-sm mb-2">Number of guests</p>
            <input type="text" placeholder="Enter Number of guests" class="w-full">

          </div>
          <div>

            <p class="text-body-sm mb-2">Date</p>
            <div x-data="datepicker()" class="relative">

              <input x-ref="dateInput" name="booking_date" type="text" class="hidden"
                     placeholder="Select Date"/>
              <div x-ref="triggerDiv">
                <div x-bind="trigger"
                     class="h-13 flex flex-col justify-center bg-white border border-secondary px-4 relative cursor-pointer select-none">

                  <div class="flex justify-between items-center">

                    <div class="flex items-center gap-2">
                      <p x-text="display" class="text-body-lg text-[#737373]"></p>
                    </div>
                    <img src="<?= get_template_directory_uri() ?>/assets/img/svg/chevron-down-black.svg" alt="Arrow icon"
                         x-bind:class="{'rotate-180': open}"
                         class="w-6 h-6 duration-300">

                  </div>

                </div>
              </div>

            </div>

          </div>
          <div class="sm:col-span-2">

            <p class="text-body-sm mb-2">Message</p>
            <textarea placeholder="Tell us about what you want from your stay..." class="w-full"></textarea>

          </div>
          <div class="sm:col-span-2">
            <button class="btn btn-primary-fill btn-lg">Make enquiry</button>
          </div>
          <div class="sm:col-span-2">
            <p class="text-body-md">By subscribing, you agree to our Terms & Conditions</p>
          </div>

        </form>

      </div>
    </div>
  </section>
  <section class="bg-secondary py-16">
    <div class="container">
      <div class="lg:w-2/3 xl:w-1/2 mx-auto">

        <div class="mb-6">

          <p class="text-body-lg text-primary text-center mb-4">SUBSCRIBE</p>
          <h3 class="text-heading-3 text-center mb-4">NEWSLETTER</h3>
          <p class="text-body-lg text-center text-[#404040]">Be the first to hear about great offers, new
                                                             openings and events.</p>

        </div>
        <div class="flex flex-row justify-center items-center mb-6">

          <input type="text" class="lg:w-80 w-56 text-body-lg" placeholder="Email address">
          <button class="btn btn-primary-fill btn-lg">Subscribe</button>

        </div>
        <p class="text-body-md text-center text-[#737373]">By subscribing, you agree to our Terms &
                                                           Conditions</p>

      </div>
    </div>
  </section>

</main>


<?php get_footer(); ?>
